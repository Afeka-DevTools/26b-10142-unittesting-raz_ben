...
PROJECT_LABEL="afeka.drupal.project=course-glossary"
...
if ! docker info >/dev/null 2>&1; then
    echo "שגיאה: Docker אינו פעיל. יש להפעיל את Docker Desktop ולנסות שוב."
    exit 1
fi

echo "מוריד את גרסאות latest העדכניות של MySQL ו-Drupal..."
docker pull "$MYSQL_IMAGE" >/dev/null
docker pull "$DRUPAL_IMAGE" >/dev/null

if docker network inspect "$NETWORK_NAME" >/dev/null 2>&1; then
    echo "הרשת $NETWORK_NAME כבר קיימת."
else
    echo "יוצר רשת Docker בשם $NETWORK_NAME..."
    docker network create --label "$PROJECT_LABEL" "$NETWORK_NAME" >/dev/null
fi

echo "מוודא שקיימים volumes של הפרויקט..."
docker volume create --label "$PROJECT_LABEL" "$MYSQL_VOLUME" >/dev/null
docker volume create --label "$PROJECT_LABEL" "$DRUPAL_VOLUME" >/dev/null
